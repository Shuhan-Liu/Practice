...
/**
 * Created by shuhanliu on 10/16/18.
 */
...
//        System.out.println(ladderLength3(beginWord, endWord, wordList));
            ...
            for (int i = 0; i < len; i++) {
                ...
                for (char n = 'a'; n <= 'z'; n++) {
                    ...
                    if (wordSet.contains(s) && !visited.contains(s)) {
                        queue.offer(new Tuple(s, t.level + 1));
                        visited.add(s);
                    }
                }
                sb.setCharAt(i, c);
    ...
    public static int ladderLength3(String beginWord, String endWord, List<String> wordList) {
        ...
        Queue<Tuple> queue = new LinkedList<>();
        queue.offer(new Tuple(beginWord, curLevel));
            ...
            for (int i = 0; i <t.word.length(); i++) {
                ...
                for (char j = 'a'; j <= 'z'; j++) {
                    ...
                    if (wordList.contains(newWord) && !set.contains(newWord)){
                        queue.offer(new Tuple(newWord, t.level + 1));
                        set.add(newWord);
                    }
                }
            }
    ...
    public static int ladderLength2(String beginWord, String endWord, List<String> wordList) {
        helper(beginWord, 1, endWord, new HashSet<String>(), wordList);
        return minStep == Integer.MAX_VALUE ? 0 : minStep;
    }

    public static void helper(String curWord, int curStep, String endWord, Set<String> visited, List<String> wordList) {
        Common.printStringHashSet(visited);
        ...
        for (int i = 0; i < curWord.length(); i++) {
            for (char j = 'a'; j <= 'z'; j++) {
                ...
                if (!visited.contains(newWord) && wordList.contains(newWord)) {
                    visited.add(newWord);
                    helper(newWord, curStep+1, endWord, visited, wordList);
    ...
    public static int ladderLength(String beginWord, String endWord, List<String> wordList) {
        ...
        findPath(beginWord, endWord, wordList, curList);

        return minLen < Integer.MAX_VALUE ? minLen : 0;
    }

    public static void findPath(String curWord, String endWord, List<String> wordList, List<String> curList) {
        for (String word : wordList) {
            if (!curList.contains(word) && calDist(curWord, word) == 1) {
                if (word.equals(endWord)) {
                    minLen = Math.min(minLen, curList.size() + 1);
                    ...
                    curList.add(word);
                    findPath(word, endWord, wordList, curList);
                    curList.remove(word);
                }
            }
        }
    ...
    public static int calDist(String w1, String w2) {
        int diff = 0;
